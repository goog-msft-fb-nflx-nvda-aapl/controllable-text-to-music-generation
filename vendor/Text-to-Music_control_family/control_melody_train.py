...
from MuseControlLite_attn_processor import (
    ...
    StableAudioAttnProcessor2_0_echo_zero,
    StableAudioAttnProcessor2_0_echo_small,
    StableAudioAttnProcessor2_0_echo_small_zero,
)
...
class CollateFunction:
    ...
    def __call__(self, examples):
        ...
        return batch
class MelodyEncoder_small(nn.Module):
    def __init__(self):
        super().__init__()
...
def main():
    ...
    transformer = pipeline.transformer.to(weight_dtype)

    # # initialize condition extractors
    condition_extractors = {}
    if "small" in config['attn_processor_type']:
        melody_conditoner = MelodyEncoder_small().cuda().float()
    else:
    ...
    processor_classes = {
        ...
        "echo_zero": StableAudioAttnProcessor2_0_echo_zero,
        "echo_small": StableAudioAttnProcessor2_0_echo_small,
        "echo_small_zero": StableAudioAttnProcessor2_0_echo_small_zero,
    }
    ...
    if config["transformer_ckpt_melody"] is not None:
        if "bin" in config["transformer_ckpt_melody"]:
            state_dict = torch.load(config["transformer_ckpt_melody"])
        elif "safetensors" in config["transformer_ckpt_melody"]:
            state_dict = load_file(config["transformer_ckpt_melody"], device="cpu")
        for name, processor in attn_procs.items():
            if isinstance(processor, attn_processor):
                if 'echo' in config["attn_processor_type"]:
                    ...
                    print(f"load {name}")
                    print(f'using {config["attn_processor_type"]}')
            if isinstance(processor, attn_processor):
